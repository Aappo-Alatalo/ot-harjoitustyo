...
class DefaultView:
    ...
    def _initialize(self):
        ...
        funds_label = ttk.Label(
            master=self._frame, text=f"Account balance: {self._portfolio_service.get_funds('Default'):.2f} €", font=('', 16))
        ...
        if investments:
            for investment in investments:
                ...
                table.insert(parent="", index=0, values=(investment.name, int(
                    investment.amount), f"{(investment.amount * current_price):.2f} €", f"{(((current_price-investment.purchase_price)/investment.purchase_price) * 100):.2f}%"))
        else:
